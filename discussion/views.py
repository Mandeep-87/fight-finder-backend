...
class ThoughtPostView(viewsets.ModelViewSet):
    ...
    # def list(self, request):
    #     if request.user.is_anonymous:
    #         data = Post.objects.all().order_by('-created_at')  # Return an empty queryset for anonymous users
    #     else:
    #         field_values = self.request.query_params.getlist('view_only', None)
            
    #         # Retrieve posts that are not hidden by the user and have not been reported more than 10 times
    #         data = Post.objects.exclude(hide_user_post=request.user.id).annotate(num_report_times=Count('report_times')).filter(num_report_times__lte=10)
            
    #         if field_values:
    #             # Filter the posts based on the specified view_only values
    #             queries = [Q(view_only=value) for value in field_values]
    #             query = queries.pop()
    ...
    #         # Filter the posts to show only the posts belonging to the logged-in user
...
class affiliationSectionView(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            affiliation_position = request.data.get('affiliation_position')
            print("------------------------",affiliation_position)
            ...
            if affiliation:
                ...
                affiliation.affiliation_position = affiliation_position
...
def handle_websocket_message(request):
    channel_layer = get_channel_layer()
    message = request.data.get('message')

    # Perform operations with the message (e.g., save it to the database)
...
class ChatListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Chat.objects.filter(receiver_id=receiver_id, sender=user)
        return queryset
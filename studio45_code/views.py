...
class LoginView(GenericAPIView):
    ...
    def post(self, request):
        ...
        response.data = {
            ...
            'dob': user.dob,
            'profile_image_update': "http://65.0.94.158:8000"+user.profile_image_update.url if user.profile_image_update else '',
...
class PersonalViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Check if the current user is the property owner, a realtor, or a loan officer
        user = request.user
        if user != instance.user and not user.user_details.filter(user_type__in=['realtor', 'loan officer']).exists():
        ...
        return Response({'data': serializer.data, 'message': 'Property Updated Successfully'})
...
class allModelDataViewSet(viewsets.ModelViewSet):
    serializer_class = Get_details_detailsSerializers
    queryset = all_events_details.objects.all().select_related('events', 'seminarnformation', 'schoolgym').order_by('-id')

    def list(self, request):
        cache_key = 'all_events_data'
        ...
        serializer = Get_details_detailsSerializers(queryset, many=True, context={'request': request})
        serialized_data = serializer.data

        # Cache the serialized data for subsequent requests
...
class IsApprovedDataViewSet(viewsets.ReadOnlyModelViewSet):
    serializer_class = Get_details_detailsSerializers
    queryset = all_events_details.objects.all().order_by('-id')

    def get_queryset(self):
        # Try to retrieve the data from cache first
        cached_data = cache.get('approved_events_data')
        if cached_data is not None:
            return cached_data

        # Data not found in cache, fetch it from the database with optimizations
        queryset = all_events_details.objects.filter(
            Q(seminarnformation__is_approved=True) |
            Q(events__is_approved=True) |
            Q(schoolgym__is_approved=True)
        ).select_related('seminarnformation', 'events', 'schoolgym').order_by('-id')

        # Store the data in cache for future use (expire in 1 hour, adjust as needed)
        cache.set('approved_events_data', queryset, 3600)
    ...
    def list(self, request):
        ...
        if page is not None:
            serializer = Get_details_detailsSerializers(page, many=True, context={'request': request})
...
class allEventsDataListAPIView(generics.ListAPIView):
    serializer_class = upcomingEvents_detailsSerializers
    queryset = all_events_details.objects.all().select_related('events', 'seminarnformation', 'schoolgym').prefetch_related('events__tags', 'seminarnformation__tags', 'schoolgym__tags').order_by('-id')
    ...
    search_fields = [
        'events__title', 'events__address', 'events__country', 'events__city',
        'seminarnformation__title', 'seminarnformation__address', 'seminarnformation__country', 'seminarnformation__city',
        ...
        'schoolgym__country', 'seminarnformation__tags__tag_name', 'schoolgym__tags__tag_name', 'schoolgym__city',
    ]
    ...
    def list(self, request, format=None):
        ...
        if year and month:
            q_date = Q(
                Q(events__start_date__year=year, events__start_date__month=month) |
                Q(seminarnformation__start_date__year=year, seminarnformation__start_date__month=month)
            )
        if judo:
            q_judo = Q(
                Q(events__martial_art_style=judo) |
        ...
        if city:
            q_city = Q(
                Q(events__city=city) |
                Q(seminarnformation__city=city) |
                Q(schoolgym__city=city)
            )
        if country:
            q_country = Q(
                Q(events__country=country) |
                Q(seminarnformation__country=country) |
                Q(schoolgym__country=country)
            )

        q_online_only = Q()
        if online_only:
            q_online_only = Q(
                Q(events__online_only=online_only) |
        ...
        ).filter(q_judo).filter(q_online_only).filter(q_date).filter(q_city).filter(q_country)
...
class events_map_location(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = all_events_details.objects.filter(
            ...
            Q(schoolgym__created_at__lte=today,)&Q (schoolgym__is_approved=True)
        )
        return queryset
    ...
    def list(self, request, format=None):
        ...
        if not upcoming_events or not distance_obj:
            ...
            if  request.GET.get('type'):
                ...
                upcoming_events = self.get_queryset().annotate(
                    distance_schoolgym=GeometryDistance("schoolgym__point", target_point),
                    distance_events=GeometryDistance("events__point", target_point),
                    distance_seminar=GeometryDistance("seminarnformation__point", target_point)
                ).annotate(
                    min_distance=Case(
                        When(distance_schoolgym__isnull=False, then=F("distance_schoolgym")),
                        When(distance_events__isnull=False, then=F("distance_events")),
                        default=F("distance_seminar"),
...
#     ordering_fields = ['created_at']
#     ordering = ['created_at']
#     filterset_fields = ['type', 'events__martial_art_style', 'seminarnformation__martial_art_style', 'schoolgym__martial_art_style']
#     search_fields = ['events__title', 'events__address', 'events__country', 'seminarnformation__title',
#                      'seminarnformation__country', 'seminarnformation__address', 'schoolgym__title',
#                      'schoolgym__address', 'schoolgym__country', 'events__martial_art_style',
#                      'seminarnformation__martial_art_style', 'schoolgym__martial_art_style',
#                      'events__tags__tag_name', 'seminarnformation__tags__tag_name', 'schoolgym__tags__tag_name']
...
#         if online_only:
#             q_online_only = Q(
#                 Q(Q(events__online_only=online_only) | Q(seminarnformation__online_only=online_only) | Q(
#                     seminarnformation__online_only=online_only) | Q(seminarnformation__online_only=online_only))
#             )

#         if unit == "km":
#             if distance_obj:
#                 distance = Distance(km=float(distance_obj))
#             else:
#                 distance = None
#         elif unit == "mi":
#             if distance_obj:
#                 distance = Distance(mi=float(distance_obj))
#             else:
#                 distance = None
#         else:
#             return Response({'error': 'Invalid unit of measurement'}, status=status.HTTP_400_BAD_REQUEST)
...
#             target_point = Point(lng_obj, lat_obj)
#             upcoming_events = self.filter_queryset(self.get_queryset()).filter(
#                 Q(events__point__distance_lte=(target_point, distance)) |
#                 Q(seminarnformation__point__distance_lte=(target_point, distance)) |
#                 Q(schoolgym__point__distance_lte=(target_point, distance))
...
#             if request.GET.get('type'):
#                 upcoming_events = self.filter_queryset(self.get_queryset()).annotate(
...
#         martial_art_style = request.GET.get('martial_art_style', None)
#         if martial_art_style:
#             upcoming_events = upcoming_events.filter(
#                 Q(events__martial_art_style=martial_art_style) |
#                 Q(seminarnformation__martial_art_style=martial_art_style) |
#                 Q(schoolgym__martial_art_style=martial_art_style)
#             )

#         upcoming_events = upcoming_events[:5]  # Apply slicing after all filters
...
class UpcomingEventsView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        martial_art_style = self.request.GET.get('martial_art_style', None)
                ...
                if 'Events' in type1:
                    if file_extension == 'csv':
                        ...
                        if serializer.is_valid():
                            ...
                            # Add is_approved and status to the data being saved
                            serializer.validated_data['is_approved'] = is_approved
                            serializer.validated_data['status'] = status

                            data_save = serializer.save()
                            data = all_events_details.objects.create(user_id=user, events=data_save, type=type1[0])
                            data.save()
                        else:
                        ...
                        try:
                            ...
                            if not image_url.strip():  # check if image_url is empty
                                ...
                                print(result,'reswulrkjdfashaeswfdash')
                    ...
                    if file_extension == 'csv':
                        ...
                        if serializer.is_valid():
                            ...
                            # Now, set the start_date and end_date fields in the serializer
                            serializer.validated_data['start_date'] = start_date
                            serializer.validated_data['end_date'] = end_date

                            sem_data = serializer.save()
                            data = all_events_details.objects.create(user_id=user, seminarnformation=sem_data, type=type1[0])
                            data.save()
                        else:
                    ...
                    if file_extension == 'csv':
                        ...
                        if serializer.is_valid():
                            ...
                            data = all_events_details.objects.create(user_id=user, schoolgym=school_data, type=type1[0])
                            data.save()
                        else:
                           return Response(serializer.errors, status=400)
                    else:
                        try:
                            ...
                            if not image_url.strip():  # check if image_url is empty
                                ...
                                result = urllib.request.urlretrieve(image_url_encoded)
                                image_file = File(open(result[0], 'rb'))
                            new_obj = SchoolGym.objects.create(image=image_url,website=row[1],owner_social_media_links=row[2],title=row[3],address=row[4],country=row[5],zip_code=row[7],owner_first_name=row[8],martial_art_style=row[9],city=row[10],owner_last_name=row[11],owner_phone_number=row[12],contact_title=row[13],owner_email=row[15],price_min_ranges=row[16],price_max_ranges=row[17],days_of_operation=row[18],hours_of_operation=row[19],introduction=row[20],special_instructions=row[22],is_approved=is_approved)
                            # print()
                            print("is that object")
                            if image_url:
                                new_obj.image.save(
                                    os.path.basename(image_url),
                                    image_file,
                            ...
                            print("cxzxzc.j,xcz vxjhhgkuxz",e)
                            # print(f"Error processing row {row}: {e}")
...
class UserbackgroundView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Userbackgroundimage.objects.filter(user=user)

...
class SchoolGymSerializers(serializers.ModelSerializer):
    image = serializers.ImageField(required=False)  # Modify the image field to be not required
    image_url = serializers.URLField(required=False, allow_blank=True)  # Allow the image_url field to be blank
    ...
    def create(self, validated_data):
        ...
        if image_url:
            try:
                ...
                instance.image.save(f"{instance.pk}.jpg", image_file)
